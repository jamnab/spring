...
Model.new(:pindoit_backup, 'Pindoit Backup') do
  database MySQL do |db|
    # To dump all databases, set `db.name = :all` (or leave blank)
    db.name               = "pindoit_production"
    db.username           = "pindoit"
    db.password           = "8twpBsPCKHq6nL6CxBaqnP4a"
    db.host               = "localhost"
    db.port               = 3306
    ...
    db.skip_tables        = ["schema_migrations"]
  ...
  store_with Local do |local|
    local.path = '~/work/database-backups/'
    local.keep = 5
  end
end

# backup perform -t pindoit_backup --config-file db/backup/config.rb
